   ...
   <body>
      <table cellspacing = "1" cellpadding = "7" border = "1">
         ...
         <tr>
            ...
            <td  valign = top align = center size = "15">
               <?php echo $tr[$i][shanovnuy]; 
               $id=$_SESSION['userid'];
               $result=$db->query("SELECT fam FROM users WHERE id='$id'");
               $res = $result->fetch(PDO::FETCH_ASSOC);
               $res2=$res['fam'];
               echo " " . $res2 . " ";
               $result=$db->query("SELECT ima FROM users WHERE id='$id'");
               $res = $result->fetch(PDO::FETCH_ASSOC);
               $res2=$res['ima'];
               echo $res2. " " . $tr[$i][uspih_reg] ;
               ?>
            </td>
            <td  class="td8">
